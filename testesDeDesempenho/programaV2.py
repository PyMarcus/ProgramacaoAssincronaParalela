# segunda versão do programa para medir o desempenho de execução
...
def main() -> None:
    ...
    th = [Thread(target=primo, kwargs={'inicio': 1, 'fim': 10000},
                              daemon=True),
          Thread(target=primo, kwargs={'inicio': 10000, 'fim': 20000},
                 daemon=True),
          Thread(target=primo, kwargs={'inicio': 20000, 'fim': 30000},
                 daemon=True),Thread(target=primo, kwargs={'inicio': 40000, 'fim': 50000},
                              daemon=True),

          ]  # daemon faz mesmo que o processo pai morra, os filhos continuem
    [tt.start() for tt in th]
    [tt.join() for tt in th]
